...
final class UserPageController extends AbstractController
{
    ...
    #[IsGranted('ROLE_USER')]
    public function index(EntityManagerInterface $em, SluggerInterface $slugger, LoggerInterface $logger, Request $request, UserPasswordHasherInterface $passwordHasher, #[Autowire('%kernel.project_dir%/public/uploads/avatar')] string $avatarDirectory): Response
    {
        $user = $this->getUser();
        ...
        if ($request->isMethod('POST')) {
            try {
                ...
                $logger->info(\sprintf(
                    'CHANGEMENT FORM COMPTE UTILISATEUR : Les données de l\'utilisateur #%s ont été mise à jour',
                    $pseudo,
                ));
                ...
                $logger->error(\sprintf(
                    'ECHEC CHANGEMENT FORM COMPTE UTILISATEUR : Les changements de données de l\'utilisateur #%s ont échoué',
                    $pseudo,
                ), ['exception' => $e]);
            }
        }
